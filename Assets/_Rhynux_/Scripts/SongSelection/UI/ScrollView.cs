...
public sealed class ScrollView : FancyScrollView<Chart> {
	...
	private void OnSelectionChanged (int _index) {
		...
		m_OnSelectionChange.OnNext (m_Charts[_index]);
	...
	public void Back() {
		OnSelectionChanged ((m_LastSelectedIndex - 1 + m_Charts.Length) % m_Charts.Length);
	}
}
